...
$(document).on('click', '#IconClassStyle', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('https://4srp-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '.GreenBTN', function(evt){
    ...
    if (Mainlocation !== null){
        ...
        if (MainspawnType == "appartment2") {
            $.post('https://4srp-spawn/spawnplayerappartment2', JSON.stringify({
                spawnloc: Mainlocation,
                apartName: apartNames,
            }));
        } else if(MainspawnType !== "appartment"){
            $.post('https://4srp-spawn/spawnplayer', JSON.stringify({
                spawnloc: Mainlocation,
                typeLoc: MainspawnType
            }));
        }else {
            $.post('https://4srp-spawn/chooseAppa', JSON.stringify({
...
function CloseAddCoord() {
    $.post('https://4srp-spawn/CloseAddCoord', JSON.stringify({}));
