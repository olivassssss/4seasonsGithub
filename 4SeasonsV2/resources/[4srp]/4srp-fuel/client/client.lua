...
-- Client Events
RegisterNetEvent('4srp-fuel:client:RefuelMenu', function()
	if Config.RenewedPhonePayment then
		if not RefuelPossible then 
			TriggerEvent('4srp-fuel:client:SendMenuToServer') 
		else 
			if Config.RenewedPhonePayment then
				if not Cancelledrefuel and not RefuelCancelled then
					if RefuelPossibleAmount then
						...
						TriggerEvent('4srp-fuel:client:RefuelVehicle', purchasetype, fuelamounttotal) 
					else
...
RegisterNetEvent('4srp-fuel:client:grabnozzle', function()
	local ped = PlayerPedId()
	...
	Citizen.CreateThread(function()
		while holdingnozzle do
			...
			if not TargetCreated then if Config.FuelTargetExport then exports['4srp-target']:AllowRefuel(true) end end
			TargetCreated = true
			if dist > 7.5 then
				if TargetCreated then if Config.FuelTargetExport then exports['4srp-target']:AllowRefuel(false) end end
				TargetCreated = true
				holdingnozzle = false
...
RegisterNetEvent('4srp-fuel:client:returnnozzle', function()
	holdingnozzle = false
	TargetCreated = false
	...
	Wait(250)
	if Config.FuelTargetExport then exports['4srp-target']:AllowRefuel(false) end
	DeleteObject(fuelnozzle)
end)
...
if Config.RenewedPhonePayment then
	RegisterNetEvent('4srp-fuel:client:phone:PayForFuel', function(amount)
		...
		if success then
			if QBCore.Functions.GetPlayerData().money['bank'] <= (GlobalTax(amount) + amount) then
				...
				TriggerServerEvent('4srp-fuel:server:PayForFuel', cost, "bank")
				RefuelPossible = true
...
RegisterNetEvent('4srp-fuel:client:FinalMenu', function(purchasetype)
	print(purchasetype)
	local money = nil
	if purchasetype == "bank" then money = QBCore.Functions.GetPlayerData().money['bank'] elseif purchasetype == 'cash' then money = QBCore.Functions.GetPlayerData().money['cash'] end
	...
	local fuel = exports['4srp-input']:ShowInput({
	...
	if fuel then
		...
		if (fuel.amount + finalfuel) >= 100 then
			...
			if GlobalTax(fuel.amount * Config.CostMultiplier) + (fuel.amount * Config.CostMultiplier) <= money then
				...
				TriggerServerEvent('4srp-fuel:server:OpenMenu', totalcost, inGasStation, false, purchasetype)
			else
	...
	if CurFuel < 95 then
		exports['4srp-menu']:createMenu({
			...
			},
			{
				...
				event = "4srp-fuel:client:FinalMenu",
				icon = "fas fa-usd",
				args = {
					{
						'cash',
...
RegisterNetEvent('4srp-fuel:client:RefuelVehicle', function(data)
		...
		if isCloseVeh() then
				...
				while refueling do
					...
					if Cancelledrefuel then
						...
						if Config.RenewedPhonePayment and purchasetype == "bank" then
							...
							TriggerServerEvent("4srp-fuel:server:phone:givebackmoney", MoneyToGiveBack)
						else
							TriggerServerEvent('4srp-fuel:server:PayForFuel', refillCostB4Tax, purchasetype)
						end
						local curfuel = GetFuel(vehicle)
			...
			QBCore.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
				...
				function()
					SetBusy(false)
					refueling = false
					if not Config.RenewedPhonePayment then TriggerServerEvent('4srp-fuel:server:PayForFuel', refillCost, purchasetype) end
					local curfuel = GetFuel(vehicle)
...
exports['4srp-target']:AddTargetModel(props, {
	options = {
		{
			num = 1,
			type = "client",
			event = "4srp-fuel:client:grabnozzle",
			icon = "fas fa-gas-pump",
			label = "Grab Nozzle",
		...
		},
		{
			num = 2,
			type = "client",
			event = "4srp-fuel:client:purchasejerrycan",
		...
		{
			num = 3, -- This is the position number of your option in the list of options in the 4srp-target context menu (OPTIONAL)
			type = "client",
			event = "4srp-fuel:client:returnnozzle",
			icon = "fas fa-hand",
			label = "Return Nozzle",
			canInteract = function()
...
CreateThread(function()
	...
	exports['4srp-target']:AddTargetBone(bones, {
		options = {
			{
				type = "client",
				action = function ()
					TriggerEvent("4srp-fuel:client:RefuelMenu")
				end,
				icon = "fas fa-gas-pump",
				label = "Insert Nozzle",
				canInteract = function()
...
-- Jerry Can --
RegisterNetEvent('4srp-fuel:jerrycan:refuelmenu', function(itemData)
	...
	if holdingnozzle then
		...
		exports['4srp-menu']:createMenu({
			...
			},
			{
				...
				icon = "fas fa-gas-pump",
				params = {
					event = "4srp-fuel:jerrycan:refueljerrycan",
					args = {
						itemData,
					},
				},
		...
		exports['4srp-menu']:createMenu({
			...
			},
			{
				...
				subheader = GasString,
				icon = "fas fa-gas-pump",
				event = "4srp-fuel:jerrycan:refuelvehicle",
				args = {
					itemData,
				},
				disabled = nogas,
...
RegisterNetEvent('4srp-fuel:client:jerrycanfinalmenu', function(purchasetype)
	Moneyamount = nil
	...
	if Moneyamount > math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice)) then
		TriggerServerEvent('4srp-fuel:server:purchase:jerrycan', purchasetype)
	else
...
RegisterNetEvent('4srp-fuel:client:purchasejerrycan', function()
	local playercashamount = QBCore.Functions.GetPlayerData().money['cash']
	exports['4srp-menu']:createMenu({
		{
		...
		{
			header = "Cash",
			subheader = "Pay with cash. <br> (You have: $" .. playercashamount .. ")",
			icon = "fas fa-usd",
			event = "4srp-fuel:client:jerrycanfinalmenu",
			args = {
				'cash'
			}
		},
...
RegisterNetEvent('4srp-fuel:jerrycan:refuelvehicle', function(data)
	local vehicle = QBCore.Functions.GetClosestVehicle()
	...
	local refuel = exports['4srp-input']:ShowInput({
	...
	if refuel then
		...
		function() -- Play When Done
			...
			TriggerServerEvent('4srp-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData, syphonData)
...
RegisterNetEvent('4srp-fuel:jerrycan:refueljerrycan', function(data)
	local itemData = data.itemData
	...
	if refuel then
		...
		function() -- Play When Done
			...
			local syphonData = data.itemData
			local srcPlayerData = QBCore.Functions.GetPlayerData()
			TriggerServerEvent('4srp-fuel:info', "add", tonumber(refuel.amount), srcPlayerData, syphonData)
			TriggerServerEvent('4srp-fuel:server:PayForFuel', tonumber(refuel.amount) * Config.CostMultiplier, "cash")
		end, function() -- Play When Cancel
			SetEntityVisible(fuelnozzle, true, 0)
			SetBusy(false)
			DeleteObject(JerrycanProp)
...
RegisterNetEvent('cdn-syphoning:syphon', function(data)
	...
	if HasSyphon then
		...
		if reason == "syphon" then
			local syphon = exports['4srp-input']:ShowInput({
			...
			if syphon then
				...
				if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
					...
					function() -- Play When Done
						...
						local syphonData = data.itemData
						local srcPlayerData = QBCore.Functions.GetPlayerData()
						TriggerServerEvent('4srp-fuel:info', "add", tonumber(syphon.amount), srcPlayerData, syphonData)
					end, function() -- Play When Cancel
						SetBusy(false)
						PoliceAlert(GetEntityCoords(PlayerPedId()))
						StopAnimTask(PlayerPedId(), Config.StealAnimDict, Config.StealAnim, 1.0)
