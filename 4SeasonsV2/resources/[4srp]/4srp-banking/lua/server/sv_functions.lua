...
RegisterServerEvent('4srp-banking:server:RefreshTransactions', function(prefix)
    local src = source
...
function RefreshTransactions(source)
    ...
    if result ~= nil then
        TriggerClientEvent("4srp-banking:client:UpdateTransactions", src, result)
    end
end

RegisterServerEvent('4srp-banking:server:CloseBankMenu', function(prefix)
