RegisterServerEvent('4srp-banking:server:Withdraw', function(account, amount, note, uuid)
    local src = source
    ...
    if(account == "business") then
        ...
        if data then
            ...
            if sM >= amount then
                TriggerEvent('4srp-banking:society:server:WithdrawMoney',src, amount, data.name)
