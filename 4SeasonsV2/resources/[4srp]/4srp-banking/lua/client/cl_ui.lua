...
function ToggleUI(atm)
    ...
    if (not bMenuOpen) then
        ...
        TriggerServerEvent("4srp-banking:server:CloseBankMenu")
        ...
        QBCore.Functions.TriggerCallback("4srp-banking:server:GetBankData", function(data, transactions)
...
RegisterNUICallback("DepositCash", function(data, cb)
    ...
    TriggerServerEvent("4srp-banking:server:Deposit", data.account, data.amount, (data.note ~= nil and data.note or ""), data.uuid)
end)
...
RegisterNUICallback("WithdrawCash", function(data, cb)
    ...
    TriggerServerEvent("4srp-banking:server:Withdraw", data.account, data.amount, (data.note ~= nil and data.note or ""), data.uuid)
end)
...
RegisterNUICallback("TransferCash", function(data, cb)
    ...
    TriggerServerEvent("4srp-banking:server:Transfer", data.citizenid, data.target, data.account, data.amount, (data.note ~= nil and data.note or ""), data.uuid)
end)
...
RegisterNUICallback("ChangeAcc", function(data, cb)
    ...
    TriggerServerEvent("4srp-banking:server:RefreshTransactions", SimpleBanking.Config["acc_using"])
    cb("ok")
end)
...
--// Net Events \\--
RegisterNetEvent("4srp-banking:client:UpdateTransactions", function(transactions)
    if (bMenuOpen) then
