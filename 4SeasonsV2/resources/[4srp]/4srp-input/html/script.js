...
const OpenMenu = (data) => {
    ...
    $("#4srp-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#4srp-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#4srp-input-form").serializeArray();
