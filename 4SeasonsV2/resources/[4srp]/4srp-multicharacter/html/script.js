    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                $.post('https://4srp-multicharacter/setupCharacters');
                setTimeout(function(){
                    setTimeout(function(){
                        $(".welcomescreen").fadeOut(0);
                        $(".SelectChar-NewMenu").fadeIn(150);
                        $(".NewbtnExpertMulti").fadeIn(150);
                        $.post('https://4srp-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://4srp-multicharacter/closeUI');
    $.post('https://4srp-multicharacter/disconnectButton');
});
...
$(document).on('click', '.New-NUI-Style', function(e){
    ...
    $(".SelectChar-DeleteChar").css({"background":"#ff0000"});
    $(".SelectChar-SpawnChar").css({"background":"#0DD1BA"});
...
$(document).on('click', '.RegisterGreen', function (e) {
    ...
    if(success){
        $.post('https://4srp-multicharacter/createNewCharacter', JSON.stringify({
            firstname: firstname,
            lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://4srp-multicharacter/removeCharacter', JSON.stringify({
...
function RefreshExpertData() {
    ...
    $(".SelectChar-header").html("Seleciona uma Personagem");
    $('.BlackScreenForNotAbuse').fadeOut(150);
    $('#Expert-woman').attr('src','https://cdn.discordapp.com/attachments/877595408325574656/878254563063369818/female.png');
    $('#Expert-man').attr('src','https://cdn.discordapp.com/attachments/877595408325574656/878254579622502471/male.png');
...
$(document).on('click', '.SelectChar-SpawnChar', function(e) {
    ...
    if (selectedChar !== null) {
        $.post('https://4srp-multicharacter/selectCharacter', JSON.stringify({
