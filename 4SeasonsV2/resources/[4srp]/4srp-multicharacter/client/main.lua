...
CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('4srp-multicharacter:client:chooseChar')
			return
		end
	end
end)
...
local function openCharMenu(bool)
    QBCore.Functions.TriggerCallback("4srp-multicharacter:server:GetNumberOfCharacters", function(result)
...
RegisterNetEvent('4srp-multicharacter:client:closeNUIdefault', function() -- This event is only for no starting apartments
    ...
    TriggerServerEvent('4srp-houses:server:SetInsideMeta', 0, false)
    TriggerServerEvent('4srp-apartments:server:SetInsideMeta', 0, 0, false)
    Wait(500)
    openCharMenu()
    ...
    TriggerEvent('4srp-clothes:client:CreateFirstCharacter')
end)

RegisterNetEvent('4srp-multicharacter:client:closeNUI', function()
    DeleteEntity(charPed)
    for k, v in pairs(NewPeds) do
        SetEntityAsMissionEntity(v[1], true, true)
        DeleteEntity(v[1])
...
RegisterNetEvent('4srp-multicharacter:client:chooseChar', function()
    SetNuiFocus(false, false)
    DoScreenFadeOut(10)
    Wait(1000)
...
RegisterNUICallback('disconnectButton', function()
    ...
    NewPeds = {}
    TriggerServerEvent('4srp-multicharacter:server:disconnect')
end)
...
RegisterNUICallback('selectCharacter', function(data)
    ...
    deleteTrain()
    DoScreenFadeOut(10)
    TriggerServerEvent('4srp-multicharacter:server:loadUserData', cData)
    openCharMenu(false)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
    for k, v in pairs(NewPeds) do
...
RegisterNUICallback('setupCharacters', function()
    ...
    NewPeds = {}
    QBCore.Functions.TriggerCallback("4srp-multicharacter:server:SetupNewCharacter", function(result)
        for k, v in pairs(result) do 
            ...
            if model ~= nil then
                CreateThread(function()
                    ...
                    TriggerEvent('4srp-clothing:client:loadPlayerClothing', data, charPed)
                    NewPeds[k] = {charPed}
                end)
            end
            Wait(20)
...
RegisterNUICallback('createNewCharacter', function(data)
    ...
    TriggerServerEvent('4srp-multicharacter:server:createCharacter', cData)
    Wait(500)
end)

RegisterNUICallback('removeCharacter', function(data)
    TriggerServerEvent('4srp-multicharacter:server:deleteCharacter', data.citizenid)
    TriggerEvent('4srp-multicharacter:client:chooseChar')
end)