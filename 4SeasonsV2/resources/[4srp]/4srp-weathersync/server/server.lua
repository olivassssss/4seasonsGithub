...
local function nextWeatherStage()
    ...
    TriggerEvent("4srp-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('4srp-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('4srp-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('4srp-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('4srp-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('4srp-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('4srp-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('4srp-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('4srp-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
