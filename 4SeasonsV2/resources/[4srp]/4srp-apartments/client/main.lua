...
local function ShowEntranceHeaderMenu()
    ...
    --DESATIVADO PORQUE SO EXISTE 1 APARTAMENTO
    --[[if IsOwned then
        headerMenu[#headerMenu+1] = {
            header = Lang:t('header.enter'),
            params = {
    ...
    headerMenu[#headerMenu+1] = {
        header = Lang:t('header.enter'),
        subheader = Lang:t('subtext.enter'),
        icon = Lang:t('icon.enter'),
        event = 'apartments:client:EnterApartment',
        args = {},
        server = false
    }
    ...
    headerMenu[#headerMenu+1] = {
        header = Lang:t('header.ring_doorbell'),
        subheader = Lang:t('subtext.ring_doorbell'),
        icon = Lang:t('icon.ring_doorbell'),
    ...
    exports['4srp-menu']:createMenu(headerMenu)
end
...
local function ShowExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        header = Lang:t('header.open_door'),
        subheader = Lang:t('subtext.open_door'),
        icon = Lang:t('icon.open_door'),
    ...
    headerMenu[#headerMenu+1] = {
        header = Lang:t('header.leave'),
        subheader = Lang:t('subtext.leave'),
        icon = Lang:t('icon.leave'),
...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    local boxData = Apartments.PolyZonesLobby
    ...
    for k, v in pairs(Apartments.PolyZonesLobby) do
        local zone = PolyZone:Create(v.coords, {
            name = boxName,
            minZ = v.minZ,
            maxZ = v.maxZ,
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside and text then
            if targetKey == "entrancePos" then
                ...
                exports['4srp-core']:DrawText(text, 'left')
            end
        else
            if targetKey == "entrancePos" then
                CloseMenuFull()
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['4srp-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                RangDoorbell = nil
                Wait(500)
                TriggerEvent('4srp-weathersync:client:DisableSync')
                Wait(100)
                TriggerServerEvent('4srp-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('4srp-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('4srp-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("QBCore:Server:SetMetaData", "currentapartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('4srp-interior:client:SetNewState', true)
            else
                TriggerEvent('4srp-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('4srp-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("4srp-apartments:returnBucket")
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(10) end
    exports['4srp-interior']:DespawnInterior(HouseObj, function()
        TriggerEvent('4srp-weathersync:client:EnableSync')
        ...
        TriggerServerEvent('4srp-apartments:server:SetInsideMeta', CurrentApartment, false)
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            QBCore.Functions.Notify(Lang:t('error.nobody_home'), "error", 3000)
            CloseMenuFull()
        else
            local apartmentMenu = {
                {
            ...
            exports['4srp-menu']:createMenu(apartmentMenu)
        end
    end, ClosestHouse)
end

function CloseMenuFull()
    exports['4srp-menu']:closeMenu()
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('4srp-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('4srp-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('4srp-spawn:client:spawndentroapartamento')
            else
                TriggerEvent('4srp-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('4srp-spawn:client:openUI', true)
            end
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if RangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            QBCore.Functions.Notify(Lang:t('error.to_far_from_door'), 'error', 3000)
            return
        end
    end
...
RegisterNetEvent('4srp-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, _)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    QBCore.Functions.Notify(Lang:t('info.at_the_door'), 'inform', 3000)
end)
...
RegisterNetEvent('apartments:client:OpenDoor', function()
    if CurrentDoorBell == 0 then
        QBCore.Functions.Notify(Lang:t('error.nobody_at_door'), 'error', 3000)
        return
    end
...
RegisterNetEvent('apartments:client:ChangeOutfit', function()
    ...
    TriggerEvent('4srp-clothing:client:openOutfitMenu')
end)

RegisterNetEvent('apartments:client:Logout', function()
    TriggerServerEvent('4srp-houses:server:LogoutLocation')
end)
