...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://4srp-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://4srp-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://4srp-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://4srp-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        $.post('https://4srp-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
    } else {
        if (selectedCam == this) {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://4srp-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://4srp-clothing/rotateLeft');
            break;
        case 87: // W
            $.post('https://4srp-clothing/CamUp');
            break;
        case 83: // S
            $.post('https://4srp-clothing/CamDown');
            break;            
    }
});
...
$(document).on('click', "#save-menu", function(e){
    ...
    QBClothing.Close();
    $.post('https://4srp-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    QBClothing.Close();
    $.post('https://4srp-clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://4srp-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://4srp-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
QBClothing.Close = function() {
    $.post('https://4srp-clothing/close');
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://4srp-clothing/saveOutfit', JSON.stringify({
        ...
        $(document).mousemove(function(e) {
            ...
            if (e.pageX < mX) {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://4srp-clothing/rotateLeftChar');
                    }
                 // moving character right
            } else {
                    if($("#rotate-char:hover").length != 0){
                        $.post('https://4srp-clothing/rotateRightChar');
                    }
            }
            mX = e.pageX
        });
...
$(document).ready(function() {
    $('#rotate-char').bind('mousewheel', function(e){
        if(e.originalEvent.wheelDelta /120 > 0) {
            ...
            $.post('https://4srp-clothing/fovup');
        }
        else{
            // console.log('scrolling down !');
            $.post('https://4srp-clothing/fovdown');
        }
    });
});
