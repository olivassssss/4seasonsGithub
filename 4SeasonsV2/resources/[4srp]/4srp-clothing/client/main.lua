...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("4srp-clothes:loadPlayerSkin")
...
RegisterNetEvent('4srp-clothing:client:getOutfits', function(requiredJob, gradeLevel)
    local gender = "male"
    if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    QBCore.Functions.TriggerCallback('4srp-clothing:server:getOutfits', function(result)
        openMenu({
...
if Config.UseTarget then
    ...
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports['4srp-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            if v.isGang then
                action = function()
                    ...
                    TriggerEvent('4srp-clothing:client:getOutfits', v.requiredJob, gradeLevel)
                end
            else
                action = function()
                    customCamLocation = v.cameraLocation
            ...
            exports['4srp-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['4srp-core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['4srp-core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['4srp-core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false
                exports['4srp-core']:HideText()
            end
        end)
            ...
            if inZone then
                ...
                elseif zoneName == 'barber' then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        TriggerEvent('4srp-clothing:client:getOutfits', clothingRoom.requiredJob, gradeLevel)
                    end
                end
            end
            Wait(sleep)
...
RegisterNetEvent('4srp-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('4srp-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('4srp-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNetEvent('4srp-clothing:client:openMenu')
AddEventHandler('4srp-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('4srp-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)

RegisterNetEvent('4srp-clothing:client:reloadOutfits')
AddEventHandler('4srp-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('4srp-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('4srp-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
    ...
    TriggerServerEvent("4srp-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('4srp-clothes:client:CreateFirstCharacter')
AddEventHandler('4srp-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("4srp-clothes:loadSkin")
AddEventHandler("4srp-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('4srp-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('4srp-clothing:client:loadPlayerClothing')
AddEventHandler('4srp-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('4srp-clothing:client:loadOutfit')
AddEventHandler('4srp-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("4srp-clothing:client:adjustfacewear")
AddEventHandler("4srp-clothing:client:adjustfacewear",function(type)
...
function reloadSkin(health)
    ...
    TriggerServerEvent("4srp-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("4srp-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
