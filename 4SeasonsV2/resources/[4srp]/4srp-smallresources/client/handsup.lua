...
RegisterCommand('hu', function()
    ...
    handsup = not handsup
    if exports['4srp-policejob']:IsHandcuffed() then return end
    if handsup then
        TaskPlayAnim(ped, animDict, anim, 8.0, 8.0, -1, 50, 0, false, false, false)
        exports['4srp-smallresources']:addDisableControls(disableHandsupControls)
    else
        ClearPedTasks(ped)
        exports['4srp-smallresources']:removeDisableControls(disableHandsupControls)
    end
end, false)
