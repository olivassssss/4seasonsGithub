...
function toggleSpectate(targetPed, targetPlayerId)
    ...
    if isSpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        QBCore.Functions.Notify('Stopped spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
        storedTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        QBCore.Functions.Notify('Started spectating '..GetPlayerName(storedTargetPlayerId)..' ('..GetPlayerServerId(storedTargetPlayerId)..')')
...
function cleanupFailedResolve()
    ...
    QBCore.Functions.Notify('Stopped spectating, could not find player..', 'error')
end
...
RegisterNetEvent('QBCore/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return QBCore.Functions.Notify('You can\'t spectate yourself..', 'error')
    end
