...
QBCore.Functions.CreateCallback("4srp-garage:server:GetGarageVehicles", function(source, cb, garage, type, category)
    local src = source
...
QBCore.Functions.CreateCallback("4srp-garage:server:validateGarageVehicle", function(source, cb, garage, type, plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if type == "public" then        --Public garages give player cars in the garage only
...
QBCore.Functions.CreateCallback("4srp-garage:server:checkOwnership", function(source, cb, plate, type, house, gang)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    elseif type == "house" then     --House garages only for player cars that have keys of the house
        MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {plate}, function(result)
            if result[1] then
                local hasHouseKey = exports['4srp-houses']:hasKey(result[1].license, result[1].citizenid, house)
                if hasHouseKey then
                    cb(true)
                else
                    cb(false)
...
QBCore.Functions.CreateCallback('4srp-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, warp)
...
QBCore.Functions.CreateCallback("4srp-garage:server:GetVehicleProperties", function(_, cb, plate)
    local properties = {}
    local result = MySQL.query.await('SELECT mods FROM player_vehicles WHERE plate = ?', {plate})
    if result[1] then
...
QBCore.Functions.CreateCallback("4srp-garage:server:IsSpawnOk", function(_, cb, plate, type)
...
RegisterNetEvent('4srp-garage:server:updateVehicle', function(state, fuel, engine, body, plate, garage, type, gang)
    QBCore.Functions.TriggerCallback('4srp-garage:server:checkOwnership', source, function(owned)     --Check ownership
        if owned then
...
RegisterNetEvent('4srp-garage:server:updateVehicleState', function(state, plate, garage)
    local type
    ...
    QBCore.Functions.TriggerCallback('4srp-garage:server:validateGarageVehicle', source, function(owned)     --Check ownership
        if owned then
...
RegisterNetEvent('4srp-garages:server:UpdateOutsideVehicle', function(plate, vehicle)
...
RegisterNetEvent('4srp-garage:server:PayDepotPrice', function(data)
    local src = source
    ...
    MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {vehicle.plate}, function(result)
        if result[1] then
            if cashBalance >= result[1].depotprice then
                ...
                TriggerClientEvent("4srp-garages:client:takeOutGarage", src, data)
...
QBCore.Functions.CreateCallback("4srp-garage:server:checkVehicleOwner", function(source, cb, plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    MySQL.query('SELECT * FROM player_vehicles WHERE plate = ? AND citizenid = ?',{plate, pData.PlayerData.citizenid}, function(result)
        if result[1] then
...
--Call from qb-phone
QBCore.Functions.CreateCallback('4srp-garage:server:GetPlayerVehicles', function(source, cb)
