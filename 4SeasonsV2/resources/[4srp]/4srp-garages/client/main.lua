...
local function MenuGarage(type, garage, indexgarage)
    ...
    exports['4srp-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "4srp-garages:client:VehicleList",
                args = {
                    type = type,
                    garage = garage,
                    index = indexgarage,
...
local function ClearMenu()
    TriggerEvent("4srp-menu:closeMenu")
end
        ...
        if isPointInside then
            ...
            elseif type == "house" then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['4srp-core']:DrawText(Lang:t("info.park_e"), 'left')
                    InputIn = true
                else
                    exports['4srp-core']:DrawText(Lang:t("info.car_e"), 'left')
                    InputOut = true
                end
            end
        else
...
RegisterNetEvent("4srp-garages:client:VehicleList", function(data)
    ...
    QBCore.Functions.TriggerCallback("4srp-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for _, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "4srp-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
                    ...
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "4srp-garages:client:takeOutGarage",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
            ...
            exports['4srp-menu']:openMenu(MenuGarageOptions)
        end
    end, indexgarage, type, garage.vehicle)
end)

RegisterNetEvent('4srp-garages:client:takeOutGarage', function(data)
    local type = data.type
    local vehicle = data.vehicle
    local garage = data.garage
    local index = data.index
    QBCore.Functions.TriggerCallback('4srp-garage:server:IsSpawnOk', function(spawn)
        if spawn then
            local location
            if type == "house" then
            ...
            QBCore.Functions.TriggerCallback('4srp-garage:server:spawnvehicle', function(netId, properties)
                local veh = NetToVeh(netId)
                QBCore.Functions.SetVehicleProperties(veh, properties)
                exports['4srp-fuel']:SetFuel(veh, vehicle.fuel)
                doCarDamage(veh, vehicle)
                TriggerServerEvent('4srp-garage:server:updateVehicleState', 0, vehicle.plate, index)
                closeMenuFull()
...
local function enterVehicle(veh, indexgarage, type, garage)
    ...
    if GetVehicleNumberOfPassengers(veh) == 0 then
        QBCore.Functions.TriggerCallback('4srp-garage:server:checkOwnership', function(owned)
            if owned then
                local bodyDamage = math.ceil(GetVehicleBodyHealth(veh))
                local engineDamage = math.ceil(GetVehicleEngineHealth(veh))
                local totalFuel = exports['4srp-fuel']:GetFuel(veh)
                TriggerServerEvent("4srp-vehicletuning:server:SaveVehicleProps", QBCore.Functions.GetVehicleProperties(veh))
                TriggerServerEvent('4srp-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, plate, indexgarage, type, PlayerGang.name)
                ...
                if plate then
                    TriggerServerEvent('4srp-garages:server:UpdateOutsideVehicle', plate, nil)
                end
...
RegisterNetEvent('4srp-garages:client:setHouseGarage', function(house, hasKey)
...
RegisterNetEvent('4srp-garages:client:houseGarageConfig', function(garageConfig)
    HouseGarages = garageConfig
end)

RegisterNetEvent('4srp-garages:client:addHouseGarage', function(house, garageInfo)
...
RegisterNetEvent('4srp-garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle

    if vehicle.depotprice ~= 0 then
        TriggerServerEvent("4srp-garage:server:PayDepotPrice", data)
    else
        TriggerEvent("4srp-garages:client:takeOutGarage", data)
    end
end)
